...
adv_directory = "ADV_data"

# Function to add the first 70% samples from a given JSON file
def add_samples_from_json(file_path, ratio=1.0):
    """Adds the first 70% samples and labels from a JSON file to the provided lists."""
    with open(file_path, "r") as f:
        data = f.readlines()  # Read all lines at once
        num_samples = int(len(data) * ratio)  # Calculate the number of samples to take
        for line in data[:num_samples]:  # Get only the first 70%
            json_object = json.loads(line)
            samples.append(json_object['func'])
            labels.append(json_object['target'])

# Loop through each file in the json_directory
for filename in os.listdir(json_directory):
    if filename.endswith(".json"):  # Only process JSON files
        file_path = os.path.join(json_directory, filename)
        add_samples_from_json(file_path)  # Add first 70% of the samples

# Optional: Now, you can add from the adv_directory in a similar way if needed
for filename in os.listdir(adv_directory):
    if filename.endswith(".json"):  # Only process JSON files
        file_path = os.path.join(adv_directory, filename)
        add_samples_from_json(file_path, ratio=0.7)
