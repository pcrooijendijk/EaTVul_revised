    ...
    def __init__(self, tokenizer, args, file_path=None):
        ...
        with open(file_path) as f:
            for line in f:
                ...
                input_feature, ids_dict = convert_examples_to_features(js,tokenizer,args)
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        for step, batch in enumerate(bar):
            ...
            loss = F.binary_cross_entropy_with_logits(logits[1].flatten(), labels.float())

            # loss = F.binary_cross_entropy_with_logits(torch.tensor(logits[1].detach().numpy(), dtype=torch.float32).flatten(), labels.float())
            ...
            # Validation
            pred = torch.max(model(inputs).detach(), dim=1)[1]
            correct += torch.sum(pred.eq(labels)).item()
            total += labels.numel()

            bar.set_description("epoch {} loss {} accuracy {}".format(idx,avg_loss,100.*correct / total))
